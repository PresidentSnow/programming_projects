...
# subnet for ipv4 class A, B, C, D and E
# and loopback, ipv4 private class A, B and C
...
# regex for IPv4 class A between 0.0.0.0-127.255.255.255 rank
REGEX_CLASS_A='^(?:(0|1[0-9]{0,2}|12[0-6])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'
# theoric rank: 0.0.0.0-127.255.255.255
# useful rank (real): 1.0.0.0-126.255.255.255

# regex for IPv4 class B between 128.0.0.0-191.255.255.255
REGX_CLASS_B='^(?:12[8-9]|1[3-8][0-9]|19[0-1])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'
# class B has no reserved rank

# regex for IPv4 class C between 192.0.0.0-223.255.255.255
REGEX_CLASS_C='^(?:19[2-9]|2[0-1][0-9]|22[0-3])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'
# class C has no reserved rank

# regex for IPv4 class D between 224.0.0.0-239.255.255.255
REGEX_CLASS_D='^(?:22[4-9]|23[0-9])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'
# reserved for multicast address, no for hosts or conventional IP address
# they are not "useful' for devices or computers

# regex for IPv4 class E between 240.0.0.0-255.255.255.255
REGEX_CLASS_E='^(?:24[0-9]|25[0-4]|255)(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'
# reserved address for research and development
# 255.255.255.255 are reserved for limited broadcast address

# regex for IPv4 loopback address between 127.0.0.0-127.255.255.255
REGEX_LOOPBACK='^(?:127)(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'

# regex for check if the IPv4 address is private and their class

# private address class A
# address range is: 10.0.0.0-10.255.255.255
REGEX_PRIVATE_A='^(?:10)(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){3}$'

# private address class B
# address range is: 172.16.0.0-172.31.255.255
REGEX_PRIVATE_B='^172\.(?:1[6-9]|2[0-9]|3[0-1])(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){2}$'

# private address class C
# address range is: 192.168.0.0-192.168.255.255
REGEX_PRIVATE_C='^192\.168\.(25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})(?:\.(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})){2}$'

# *NOTE*; this part: 1?[0-9]{1,2}, explained step by step

# 1?: this means "zero or nothing (empty string)'
# [0-9]{1,2}: matches any digit between 0-9 and {1,2} does mean
# preceding element (in this case [0-9]) must appear at least 1
# time and at most 2 times

# Funcion para clasificar la IP
classify_ip() {
  local ip=$1
...
# Function for convert IP number to 32 bits
ip_to_num() {
    local ip=$1
...
# Funcion mejorada para calcular informacion de red
calculate_network() {
  local ip=$1
  local cidr=$2

  # Special case for CIDR 0 (red 0.0.0.0/0)
  if [[ "$cidr" -eq 0 ]]; then
    echo "0.0.0.0/0"
    echo "0.0.0.0"
...
# Main function for subnetting
perform_subnetting() {
    local network_ip=$1
    local original_cidr=$2
    local num_subnets=$3

    # Calcular nuevos bits de mascara
...
# Funcion para solicitar entrada al usuario
get_user_input() {
    while true; do
